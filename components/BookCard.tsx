...
import { getBookPlot } from '../services/geminiService';
...
const BookCard: React.FC<BookCardProps> = ({ book }) => {
  ...
  const openPlotModal = async () => {
    ...
    setModalContent(
      <div className="flex flex-col items-center justify-center py-8">
        ...
        <p className="text-gray-400 animate-pulse">Generating plot summary with Gemini...</p>
      </div>
    );
    
    setIsLoadingPlot(true);
    const plot = await getBookPlot(book.title, book.authors[0]);
    setIsLoadingPlot(false);
    
    setModalContent(
      <div className="text-gray-200 leading-relaxed space-y-4">
        <p>{plot}</p>
        <div className="pt-4 mt-4 border-t border-gray-700 text-xs text-gray-500">
          Generated by Gemini
        </div>
      </div>
    );
  ...
  const openGenreModal = () => {
    setModalTitle(`Genres: ${book.title}`);
    setModalContent(
      <div className="flex flex-wrap gap-2">
        {book.subjects && book.subjects.length > 0 ? (
          book.subjects.map((subject, idx) => (
            <span key={idx} className="bg-gray-700 text-gray-200 px-3 py-1 rounded-full text-sm border border-gray-600">
              {subject}
            </span>
          ))
        ) : (
  ...
  return (
    <>
      <div className="group flex flex-col bg-gray-800 rounded-lg overflow-hidden shadow-lg hover:shadow-indigo-500/30 transition-all duration-300 transform hover:-translate-y-1 h-full">
        ...
        <div className="p-4 flex flex-col flex-grow">
          ...
          {book.subjects && book.subjects.length > 0 && (
              <p className="text-xs text-gray-500 mt-1 truncate mb-3" title={book.subjects.join(', ')}>
          ...
          <div className="mt-auto flex gap-2 pt-2">
            <button
                ...
                className="flex-1 bg-indigo-900/50 hover:bg-indigo-900 text-indigo-200 text-xs font-semibold py-2 px-2 rounded border border-indigo-700/50 hover:border-indigo-500 transition-all"
            >
                Plot
            </button>
             <button
                onClick={openGenreModal}
                className="flex-1 bg-gray-700/50 hover:bg-gray-700 text-gray-300 text-xs font-semibold py-2 px-2 rounded border border-gray-600/50 hover:border-gray-500 transition-all"
            >
                Genres
            </button>
          </div>
